<?php
$post = mysqli_query($con,"SELECT * FROM blog_vaneskha_gallery ORDER BY id DESC");
$comment = mysqli_query($con,"SELECT blog_vaneskha_gallery_comment.* , blog_vaneskha_gallery.title
...
if(isset($_POST["submit"])){
    ...
    if($_POST["status"]== 1){
        $post_slt = mysqli_query($con,"SELECT * FROM blog_vaneskha_gallery_comment WHERE post_id = '$post' AND status = 1 ");
        $cur_cmt = mysqli_num_rows($post_slt);
        
        $cur_stat = $cur_cmt+1;
        mysqli_query($con,"UPDATE blog_vaneskha_gallery SET fav = '$cur_stat' WHERE id = '$post'");
        mysqli_query($con,"INSERT INTO blog_vaneskha_gallery_comment VALUES ('','$post','$name','$reply','$status','$date')");
        header("location:index.php?gal-comment");
    }else{
    mysqli_query($con,"INSERT INTO blog_vaneskha_gallery_comment VALUES ('','$post','$name','$reply','$status','$date')");
    header("location:index.php?gal-comment");
    }
...
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">Gallery &raquo; Comment</h1>
    </div>
</div>
<div class="row">
    <div class="col-lg-12">
                    ...
                    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="center"><a href="index.php?gal-cmt-del=<?php echo $co_row["id"]?> " class="btn btn-primary btn-xs" type="button">Delete</a></td>
                            </tr>
                                <?php }?>
                            <?php }?>
                        </tbody>
